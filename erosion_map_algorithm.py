...
class ErosionMapAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterMultipleLayers('old_point_cloud(s)', 'Old Point Cloud(s)', layerType=QgsProcessing.TypePointCloud, defaultValue=None))
        self.addParameter(QgsProcessingParameterMultipleLayers('new_point_cloud(s)', 'New Point Cloud(s)', layerType=QgsProcessing.TypePointCloud, defaultValue=None))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
       ...
       # Old Merge
        alg_params = {
            'FILTER_EXPRESSION': '',
            'FILTER_EXTENT': None,
            'LAYERS': parameters['old_point_cloud(s)'],
            'OUTPUT': '{0}\\old_point_cloud.las'.format(parameters['data_export_destination'])
        }
        outputs['old_point_cloud'] = processing.run('pdal:merge', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            'FILTER_EXPRESSION': '',
            'FILTER_EXTENT': None,
            'LAYERS': parameters['new_point_cloud(s)'],
            'OUTPUT': '{0}\\new_point_cloud.las'.format(parameters['data_export_destination'])
        }
        outputs['new_point_cloud'] = processing.run('pdal:merge', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            ...
            'FILTER_EXPRESSION': '',
            'FILTER_EXTENT': None,
            'INPUT': '{0}\\old_point_cloud.las'.format(parameters['data_export_destination']),
            'ORIGIN_X': None,
            'ORIGIN_Y': None,
        ...
        # Old Assign projection
        alg_params = {
            'CRS': 'ProjectCrs',
            'INPUT': '{0}\\Old.tif'.format(parameters['data_export_destination'])
        }
        outputs['OldAssignProjection'] = processing.run('gdal:assignprojection', alg_params, context=context, feedback=feedback, is_child_algorithm=True)

        feedback.setCurrentStep(6)
        if feedback.isCanceled():
            return {}

        # New Assign projection
        alg_params = {
            'CRS': 'ProjectCrs',
            'INPUT': '{0}\\New.tif'.format(parameters['data_export_destination'])
        }
        outputs['NewAssignProjection'] = processing.run('gdal:assignprojection', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            ...
            'STYLE': '{0}python\\plugins\\qgis_Erosion_Map-main\\style.qml'.format(QgsApplication.qgisSettingsDirPath()),
